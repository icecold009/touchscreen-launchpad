...
- Scope: Use compact hardware-inspired rotary faces with fixed ticks and moving indicators; keep 44px touch targets; give master volume a real vertical fader; flatten nested mixer framing; retain a clear performance-first flow, restrained per-section accents, accessible navigation, and existing pad colors; characterize and isolate IndexedDB/local-settings adapters without changing persistence contracts; update static app-shell asset versions.
- Non-goals: Change audio values or DSP, add MIDI features, replace native range inputs, change persisted data formats or ownership, or claim physical touchscreen/device evidence.
- Files: `index.html`, `style.css`, `app.js`, `src/bootstrap.js`, `src/storage/indexed-db.js`, `src/storage/local-settings.js`, `sw.js`, `test/fixtures/storage-contract.mjs`, `test/storage-repository.test.mjs`, `test/kit-library.test.mjs`, `test/module-contract.test.mjs`, `test/pwa.test.mjs`, `test/service-worker-runtime.test.mjs`, `test/storage-recovery.test.mjs`, `test/site-dom.test.mjs`, and this backlog entry.
- Tests: `npm.cmd run validate`, `git diff --check`, and localhost render/navigation smoke at the browser's 1086×610 viewport; verify transport → pad bank → kit → arrangement → capture order, Mix/Arrange jumps, map expand/collapse, labeled FX groups, dial dimensions, responsive CSS, requested assets, and the sequencer play/stop loop. This pass does not claim a separately emulated mobile/device run or physical pointer/touch hardware proof.
- Acceptance: Core pads appear immediately after transport; section jumps are directly available; twelve mixer knobs use compact ~30px faces within 44px interaction areas; master volume is a labeled vertical fader; centered controls preserve neutral markers; FX groups scan without nested card boxes; values stay legible; all 16 pads, local-first state, audio, MIDI, sequencer, storage, keyboard, and pointer behavior remain intact; no console errors or horizontal overflow.
- Evidence: Authenticated Jev reviews informed compact charcoal rotary controls with fixed ticks and moving indicators, amber active illumination, neutral-family tints for default pads, performance-first transport/pad order, in-place sequencer playhead updates, and direct scene/play/swing controls with infrequent scene tools disclosed. Further Jev-guided refinements flattened the capture/master bus and selected-step editor into the chassis; the latter retains Track, Step, Probability, and Micro timing, and the selected scene now uses an amber depressed hardware state. The transport strip now removes its inherited inline border and radius while retaining thin top/bottom dividers. At <=380px the mixer becomes one column so three 44px knob targets fit without squeezing; CSS contract tests cover the responsive pad, sequencer, mixer, editor, and transport chassis rules. Persistence repository contracts characterize the browser-local read/write/recovery behavior, and IndexedDB/local settings are isolated behind adapters without changing saved formats. Existing user pad colors and browser-local records were preserved. `npm.cmd run validate` passes all 90 tests; `git diff --check` passes with only the repository's CRLF-to-LF normalization warnings. The localhost render at 1086×600 confirms the slightly larger white edge tick on non-mixer knobs while the compact mixer ticks retain their prior size; localhost and `style.css?version=82` return HTTP 200. The PWA stylesheet precache is synchronized at service-worker cache v83. This final browser pass did not capture page console logs, a 320px emulated viewport, physical touchscreen input, hosted verification, or deployment evidence.
