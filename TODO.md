...
- Goal: Replace repeated AI-dashboard styling with a compact, tactile sampler performance console without changing audio behavior or local data contracts.
- Scope: Use compact hardware-inspired rotary faces with fixed ticks and moving indicators; keep 44px touch targets; give master volume a real vertical fader; flatten nested mixer framing; retain a clear performance-first flow, restrained per-section accents, accessible navigation, and existing pad colors; update static app-shell asset versions.
- Non-goals: Change audio values or DSP, add MIDI features, replace native range inputs, change local persistence or data contracts, or claim physical touchscreen/device evidence.
- Files: `index.html`, `style.css`, `app.js`, `src/bootstrap.js`, `src/storage/indexed-db.js`, `sw.js`, `test/site-dom.test.mjs`, `test/module-contract.test.mjs`, and this backlog entry.
...
- Evidence: Authenticated Jev reviews informed compact charcoal rotary controls with fixed ticks and moving indicators, amber active illumination, neutral-family tints for default pads, performance-first transport/pad order, in-place sequencer playhead updates, and direct scene/play/swing controls with infrequent scene tools disclosed. Further Jev-guided refinements flattened the capture/master bus and selected-step editor into the chassis; the latter retains Track, Step, Probability, and Micro timing, and the selected scene now uses an amber depressed hardware state. The transport strip now removes its inherited inline border and radius while retaining thin top/bottom dividers. At <=380px the mixer becomes one column so three 44px knob targets fit without squeezing; CSS contract tests cover the responsive pad, sequencer, mixer, editor, and transport chassis rules. Existing user pad colors and browser-local records were preserved. `npm.cmd run validate` passes all 90 tests; `git diff --check` has only the repository's CRLF-to-LF normalization warnings. The localhost browser render at 1086×610 visibly shows all 16 family-tinted tactile pads, the master fader at its original 80%, compact transport controls, four 16-step tracks, an amber selected scene, a flat step-detail strip, and the shared capture/mixer chassis. Scene tools remains in document flow with scene names, launch/chain settings, clear/duplicate/undo/redo, and export/render controls. Sequencer Play/Stop and playhead cleanup were verified in the browser in the preceding iteration. Stylesheet URL is `style.css?version=79`; the service-worker precache is synchronized at cache v80, and localhost returns HTTP 200. This browser session did not capture page console logs, a 320px emulated viewport, physical touchscreen input, hosted verification, or deployment evidence.
